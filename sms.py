...
def connect_database():
    def connect():
        ...
        try:
            ...
            mycursor.execute(query)
            query = "create table student(id int not null primary key, name varchar(30), mobile varchar(15), email varchar(100), address varchar(100), gender varchar(20), dob varchar(20), date varchar(50), time varchar(50))"
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
...
def clock():
    ...
    currenttime=time.strftime('%I:%M:%S%p') #12-hour format
